...
import storm.starter.trident.project.countmin.SketchComparator;
import java.io.*;
import java.util.*;
 ...
 * http://www.eecs.harvard.edu/~michaelm/CS222/countmin.pdf Modified by Preetham
 * MS. Originally by https://github.com/addthis/stream-lib/
 *
...
public class CountMinSketchState implements State {
    ...
    public CountMinSketchState(int depth, int width, int seed, int k) {
        ...
        // initialize a priority queue with size k
        topList = new TopList(k, this);
    }
    ...
    public CountMinSketchState(double epsOfTotalCount, double confidence, int seed) {
        ...
        initTablesWith(depth, width, seed);
        //this.topList = new TopList(k, table);
    }
    ...
    //  gets top items from priority queue
    public List<String> getTopKitems() {
        return topList.printTopKItems();
    }
     ...
     * @throws CMSMergeException if estimators are not mergeable (same depth,
     * width and seed)
     */
