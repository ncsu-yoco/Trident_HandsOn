 ...
 * fake tweets and how to query it, using DRPC calls. This example should be
 * intended as an example of {@link TridentState} custom implementation.
 * Modified by Kapil Somani(kmsomani@ncsu.edu) from original author:
 *
...
public class RealTimeElasticSearchTopology {
    ...
    public static StormTopology buildTopology(String[] args, Settings settings, LocalDRPC drpc)
        ...
        // starting ElasticSearch with specified settings
        ...
        /*
         we are selecting following list of words as the Elastic Search queries are based on them
         */
        String[] topicWords = {"love", "hate", "man", "moon", "apples", "very", "like"};

        // Create Twitter's spout
        // Tweets will be fetched using twitter API in real time.
        //
         ...
         * Change setCycle(false) to setCycle(true) if continuous stream of
         * tweets is needed
         *
         */
        /*
         ...
         */
        /**
         * Read a stream of tweets, parse and extract only the text, and its id.
         * Store the stream using the {@link ElasticSearchState} implementation
         * for Trident. NOTE: Commented lines are for debugging only TO DO FOR
         * PROJECT 2: PART B: 1. Write a spout that ingests real text data
         * stream: You are allowed to utilize TweeterSampleSpout provided
         * (DEAFULT), or you may consider writing a spout that ingests Wikipedia
         * updates or any other text stream (e.g., using Google APIs to bring a
         * synopsis of a URL web-page for a stream of URL web-pages). 2. Write
         * proper support/utility functions to prepare your text data for
         * passing to the ES stateFactory: e.g., WikiUpdatesDocumentBuilder(),
         ...
         ***/
///*
        topology.newStream("tweets", spoutTweets) // declare which stream to read from
                .parallelismHint(1)
                ...
                .each(new Fields("text", "tweetId", "user"), new SentenceBuilder(), new Fields("sentence")) //extract the text content from the tweet
                .each(new Fields("sentence"), new DocumentBuilder(), new Fields("document"))
                .each(new Fields("document"), new ExtractDocumentInfo(), new Fields("id", "index", "type"))
                //.each(new Fields("id"), new Print("id field"))
                .groupBy(new Fields("index", "type", "id"))
...
//*/
         ...
         */
        topology
                .newDRPCStream("search_event", drpc)    // on event handler
                .each(new Fields("args"), new ExtractSearchArgs(), new Fields("query", "indices", "types")) // extract relevant information from passed query
                .groupBy(new Fields("query", "indices", "types"))
                .stateQuery(staticState, new Fields("query", "indices", "types"), new QuerySearchIndexQuery(), new Fields("tweet")) // query elastic search with given query and parameters
                .each(new Fields("tweet"), new FilterNull())    // remove null results
                .each(new Fields("tweet"), new CreateJson(), new Fields("json"))    // return result as JSON object
                .project(new Fields("json"));   // project results for printing
    ...
    public static void main(String[] args) throws Exception {
        ...
        /**
         * Configure ElasticSearch related information Make sure that
         * elasticsearch (ES) server is up and running Check README.md file on
         * how to install and run ES Note that you can check if index exists
         * and/or was created with curl 'localhost:9200/_cat/indices?v' and
         * type: curl ‘http://127.0.0.1:9200/my_index/_mapping?pretty=1’
         ...
         * Remove else{} stmt if you do not want index to be deleted but rather
         * updated NOTE: Index files are stored by default under
         * $HOME/elasticsearch-1.4.2/data/elasticsearch/nodes/0/indices
         *
         */
        ...
        /**
         * *
         * TO DO FOR PROJECT 2: PART B: 3. Investigate Java APIs for
         * ElasticSearch for various types of queries:
         ...
         * and test them with the DRPC execute(). Example queries: matchQuery(),
         * multiMatchQuery(), fuzzyQuery(), etc. 5. Can you thing of the type of
         * indexing technologies ES might be using to support efficient query
         * processing for such query types: e.g., fuzzyQuery() vs. boolQuery().
         * *
         */
        String query1 = QueryBuilders.termQuery("text", "man").buildAsBytes().toUtf8();
        String query2 = QueryBuilders.wildcardQuery("text", "*er*").buildAsBytes().toUtf8();
        String query3 = QueryBuilders.termsQuery("text", "love", "hate", "man", "moon", "apples", "very", "like").minimumMatch(2).buildAsBytes().toUtf8();
        String query4 = QueryBuilders.fuzzyQuery("text", "RT").buildAsBytes().toUtf8();
        String query5 = QueryBuilders.matchQuery("text", "hate").buildAsBytes().toUtf8();
        String query6 = QueryBuilders.boolQuery()
